...
const Signup = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (error) {
      ...
      toast.success("Đã tạo tài khoản! Bạn đã đăng nhập.");
      navigate(next);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center p-4 bg-background">
      <Card className="w-full max-w-md">
        <CardHeader className="text-center">
          ...
          <CardDescription>Tham gia Easybooko và bắt đầu đặt phòng</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <Button variant="outline" className="w-full" onClick={() => signInWithGoogle(next)}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Input type="password" placeholder="Mật khẩu (tối thiểu 8 ký tự)" value={password} onChange={(e) => setPassword(e.target.value)} minLength={8} required />
